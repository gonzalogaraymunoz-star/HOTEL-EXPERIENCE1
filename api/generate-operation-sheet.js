...
function accommodation(lead){return lead?.hotel_partners?.partner_type==='hotel'?lead.hotel_partners.name||'':'';}
function cell(sheet,address,value,date=false){
  if(!sheet||value==null||value==='') return;
  const c=sheet.getCell(address);c.value=value;if(date)c.numFmt='dd-mm-yyyy';
}
function clearRange(sheet,startRow,endRow,startCol,endCol){
  if(!sheet)return;for(let r=startRow;r<=endRow;r++)for(let c=startCol;c<=endCol;c++)sheet.getCell(r,c).value=null;
}
function colName(n){let out='';let value=n;while(value>0){const rem=(value-1)%26;out=String.fromCharCode(65+rem)+out;value=Math.floor((value-1)/26);}return out;}
function serviceText(service,product){return normalize([service.producto,service.service_type,service.tour_id,product?.name,product?.product_slug,product?.stops,product?.code].filter(Boolean).join(' '));}
...
function assignmentPeople(assignment,peopleById){
  ...
  return {guideName:guide?.full_name||assignment?.guide_name||'',guideRut:guide?.rut||'',guideSernatur:guide?.sernatur_registration||'',guidePhone:guide?.phone||guide?.whatsapp||'',driverName:driver?.full_name||assignment?.driver_name||'',driverRut:driver?.rut||'',driverPhone:driver?.phone||driver?.whatsapp||''};
}
function participantsFor(service,linksByService,passengers){
  ...
  const byId=new Map(passengers.map(p=>[p.id,p]));return links.map(link=>byId.get(link.passenger_id)).filter(Boolean);
}
function fillDatosPax(sheet,passengers,lead){
  clearRange(sheet,4,80,1,12);
  passengers.forEach((p,index)=>{const row=4+index;const name=splitName(p);const age=ageAt(p.birth_date,lead.checkin);cell(sheet,`A${row}`,index+1);cell(sheet,`B${row}`,name.first);cell(sheet,`C${row}`,name.last);cell(sheet,`D${row}`,p.document_number||'');cell(sheet,`E${row}`,excelDate(p.birth_date),true);cell(sheet,`F${row}`,p.nationality||'');cell(sheet,`G${row}`,accommodation(lead));cell(sheet,`H${row}`,p.dietary_restrictions||'');cell(sheet,`I${row}`,p.disability_type||'');cell(sheet,`J${row}`,mark(gender(p)==='M'));cell(sheet,`K${row}`,mark(gender(p)==='F'));cell(sheet,`L${row}`,age);});
}
function fillLuna(sheet,service,pax,ctx){
  clearRange(sheet,11,80,3,19);cell(sheet,'F6',ctx.supplier?.name||'');cell(sheet,'F7',ctx.supplier?.sernatur_registration||'');cell(sheet,'J6',ctx.people.guideName);cell(sheet,'J7',ctx.people.guideSernatur||ctx.people.guideRut);cell(sheet,'R6',ctx.vehicle?.plate||'');cell(sheet,'R7',[ctx.vehicle?.brand,ctx.vehicle?.model].filter(Boolean).join(' ')||ctx.vehicle?.label||'');
  pax.forEach((p,index)=>{const r=11+index;const name=splitName(p);const age=ageAt(p.birth_date,service.fecha_servicio);cell(sheet,`C${r}`,name.first);cell(sheet,`D${r}`,name.last);cell(sheet,`F${r}`,p.nationality||'');cell(sheet,`H${r}`,p.document_number||'');cell(sheet,`J${r}`,mark(gender(p)==='F'));cell(sheet,`K${r}`,mark(gender(p)==='M'));cell(sheet,`L${r}`,mark(gender(p)&&!['M','F'].includes(gender(p))));cell(sheet,`M${r}`,mark(age!=null&&age<18));cell(sheet,`N${r}`,mark(age!=null&&age>=18&&age<=35));cell(sheet,`O${r}`,mark(age!=null&&age>=36&&age<=59));cell(sheet,`P${r}`,mark(age!=null&&age>=60));const disability=normalize(p.disability_type);cell(sheet,`R${r}`,mark(disability.includes('fis')));cell(sheet,`S${r}`,mark(disability.includes('sens')));});
}
function fillD80(sheet,service,pax,ctx,lead){
  clearRange(sheet,13,90,2,10);['D4','D5','E5','D6','E6','D7','E7','D8','D9','D10'].forEach(a=>sheet.getCell(a).value=null);cell(sheet,'D4',excelDate(service.fecha_servicio),true);cell(sheet,'D5',ctx.people.driverName);cell(sheet,'E5',ctx.people.driverRut);cell(sheet,'D6',ctx.vehicle?.plate||'');cell(sheet,'E6',[ctx.vehicle?.brand,ctx.vehicle?.model].filter(Boolean).join(' ')||ctx.vehicle?.label||'');cell(sheet,'D7',ctx.people.guideName);cell(sheet,'E7',ctx.people.guideRut);cell(sheet,'D8',ctx.people.guideSernatur);cell(sheet,'D9',ctx.supplier?.rut||'');cell(sheet,'D10',ctx.supplier?.sernatur_registration||'');
...
function fillChaxa(sheet,service,pax,ctx){
  clearRange(sheet,14,90,3,25);cell(sheet,'H7',ctx.supplier?.name||'');cell(sheet,'S7',excelDate(service.fecha_servicio),true);cell(sheet,'H8',ctx.people.guideName);if(ctx.people.driverName)cell(sheet,'S8',`Conductor: ${ctx.people.driverName}`);cell(sheet,'H9',ctx.supplier?.name||'');if(ctx.vehicle?.plate)cell(sheet,'S9',`Patente de Vehículo: ${ctx.vehicle.plate}`);
...
function fillSocaire(sheet,service,pax,ctx,lead){
  clearRange(sheet,17,80,3,16);['E5','E6','J6','E8','H8','M8','E10','J10','E12','I12','M12','E13','I13','M13'].forEach(a=>sheet.getCell(a).value=null);cell(sheet,'E5',ctx.supplier?.name||'');cell(sheet,'E6',ctx.supplier?.rut||'');cell(sheet,'J6',ctx.supplier?.sernatur_registration||'');cell(sheet,'E8',excelDate(service.fecha_servicio),true);cell(sheet,'H8',service.producto||'');cell(sheet,'M8',service.hora_inicio?String(service.hora_inicio).slice(0,5):ctx.assignment?.pickup_time?String(ctx.assignment.pickup_time).slice(0,5):'');cell(sheet,'E10',ctx.vehicle?.plate||'');cell(sheet,'J10',[ctx.vehicle?.brand,ctx.vehicle?.model].filter(Boolean).join(' ')||ctx.vehicle?.label||'');cell(sheet,'E12',ctx.people.driverName);cell(sheet,'I12',ctx.people.driverRut);cell(sheet,'M12',ctx.people.driverPhone);cell(sheet,'E13',ctx.people.guideName);cell(sheet,'I13',ctx.people.guideRut);cell(sheet,'M13',ctx.people.guidePhone);
...
function fillTalabre(sheet,service,pax,ctx,lead){
  clearRange(sheet,21,90,2,15);for(let r=8;r<=16;r++)for(let c=3;c<=15;c++)sheet.getCell(r,c).value=null;cell(sheet,'C8',excelDate(service.fecha_servicio),true);cell(sheet,'C9',ctx.supplier?.name||'');cell(sheet,'C10',ctx.supplier?.rut||'');cell(sheet,'C11',ctx.people.driverName);cell(sheet,'C12',ctx.people.guideName);cell(sheet,'C13',ctx.vehicle?.plate||'');cell(sheet,'C14',ctx.supplier?.sernatur_registration||'');cell(sheet,'C15',ctx.supplier?.phone||ctx.supplier?.whatsapp||'');cell(sheet,'C16',ctx.supplier?.email||'');
...
function fillTransfer(sheet,service,pax,ctx,lead){
  clearRange(sheet,13,90,2,11);for(let r=2;r<=10;r++)if(![9,10].includes(r))sheet.getCell(`D${r}`).value=null;cell(sheet,'D2',excelDate(service.fecha_servicio),true);cell(sheet,'D3',service.producto||'Transfer');cell(sheet,'D4',ctx.assignment?.meeting_point||lead.pickup_location||accommodation(lead));cell(sheet,'D5',ctx.people.driverName);cell(sheet,'D6',ctx.vehicle?.plate||'');cell(sheet,'D7',ctx.people.guideName);cell(sheet,'D8',ctx.people.guideSernatur);cell(sheet,'D9',ctx.supplier?.rut||'');cell(sheet,'D10',ctx.supplier?.sernatur_registration||'');
...
function duplicateSheet(workbook,source,name){
  try{const model=structuredClone(source.model);const target=workbook.addWorksheet(name);model.id=target.id;model.name=name;target.model=model;return target;}catch{return null;}
}
function makeCopies(workbook,base,count,prefix){
  if(!base||count<=0)return[];const sheets=[base];for(let i=1;i<count;i++){const copy=duplicateSheet(workbook,base,`${prefix} ${i+1}`);if(copy)sheets.push(copy);}return sheets;
}
function riskCol(base,n){const start=base==='B'?2:base==='L'?12:22;return colName(start+n);}
function fillRiskBlock(sheet,base,p,service,ctx,lead){
  const col=n=>riskCol(base,n);const name=splitName(p);cell(sheet,`${col(2)}8`,name.first);cell(sheet,`${col(3)}8`,name.last);cell(sheet,`${col(6)}8`,p.nationality||'');cell(sheet,`${col(2)}9`,p.document_number||'');cell(sheet,`${col(4)}9`,ageAt(p.birth_date,service.fecha_servicio));cell(sheet,`${col(2)}13`,service.producto||'Tours in San Pedro');cell(sheet,`${col(6)}13`,excelDate(service.fecha_servicio),true);cell(sheet,`${col(2)}14`,ctx.people.guideName);cell(sheet,`${col(6)}14`,ctx.people.guideRut||ctx.people.guideSernatur);cell(sheet,`${col(2)}15`,service.hora_inicio?String(service.hora_inicio).slice(0,5):ctx.assignment?.pickup_time?String(ctx.assignment.pickup_time).slice(0,5):'');cell(sheet,`${col(4)}15`,service.hora_fin?String(service.hora_fin).slice(0,5):'');cell(sheet,`${col(6)}15`,ctx.assignment?.meeting_point||lead.pickup_location||accommodation(lead));cell(sheet,`${col(8)}15`,service.producto||'');
}
function clearRiskBlock(sheet,base){const col=n=>riskCol(base,n);[`${col(2)}8`,`${col(3)}8`,`${col(6)}8`,`${col(2)}9`,`${col(4)}9`,`${col(2)}13`,`${col(6)}13`,`${col(2)}14`,`${col(6)}14`,`${col(2)}15`,`${col(4)}15`,`${col(6)}15`,`${col(8)}15`].forEach(a=>sheet.getCell(a).value=null);}
function fillRiskPackets(workbook,source,tourContexts,lead){
  if(!source||!tourContexts.length){if(source)workbook.removeWorksheet(source.id);return;}
  const packets=[];tourContexts.forEach(ctx=>{for(let i=0;i<ctx.pax.length;i+=3)packets.push({ctx,pax:ctx.pax.slice(i,i+3)});});
  const sheets=makeCopies(workbook,source,packets.length,'RIESGO');
  sheets.forEach((sheet,index)=>{['B','L','V'].forEach(base=>clearRiskBlock(sheet,base));const packet=packets[index];if(!packet)return;packet.pax.forEach((p,pIndex)=>fillRiskBlock(sheet,['B','L','V'][pIndex],p,packet.ctx.service,packet.ctx,lead));});
}
async function authenticatedAdmin(req){
  const url=process.env.SUPABASE_URL||process.env.VITE_SUPABASE_URL||'https://lpirjwifzosdzgdncsbt.supabase.co';const key=process.env.SUPABASE_SERVICE_ROLE_KEY;if(!url||!key)throw Object.assign(new Error('Faltan variables privadas de Supabase.'),{status:500});const admin=createClient(url,key,{auth:{persistSession:false,autoRefreshToken:false}});const token=(req.headers.authorization||'').replace(/^Bearer\s+/,'');const {data,error}=await admin.auth.getUser(token);if(error||!data.user)throw Object.assign(new Error('Sesión inválida.'),{status:401});const {data:profile}=await admin.from('profiles').select('role,is_active').eq('id',data.user.id).single();if(!profile?.is_active)throw Object.assign(new Error('Cuenta desactivada.'),{status:403});if(!['admin','manager','agent'].includes(String(profile.role)))throw Object.assign(new Error('Sin permiso para generar documentación.'),{status:403});return{admin,user:data.user};
}
...
export default async function handler(req,res){
  ...
  try{
    ...
    const [{data:lead,error:leadError},{data:services,error:serviceError},{data:passengers,error:paxError}]=await Promise.all([admin.from('leads').select('*,hotel_partners(name,partner_type)').eq('id',leadId).single(),admin.from('lead_services').select('*').eq('lead_id',leadId).in('booking_status',['confirmed','completed']).order('fecha_servicio').order('created_at'),admin.from('passengers').select('*').eq('lead_id',leadId).order('is_primary',{ascending:false}).order('passenger_code')]);
    if(leadError)throw leadError;if(serviceError)throw serviceError;if(paxError)throw paxError;if(!services?.length)return res.status(409).json({error:'El Lead no tiene servicios confirmados en Operaciones.'});
    const serviceIds=services.map(s=>s.id);const [{data:links,error:linkError},{data:assignments,error:assignmentError}]=await Promise.all([admin.from('lead_service_passengers').select('*').in('lead_service_id',serviceIds),admin.from('service_assignments').select('*').in('lead_service_id',serviceIds)]);if(linkError)throw linkError;if(assignmentError)throw assignmentError;
    const ids=list=>Array.from(new Set(list.filter(Boolean)));const supplierIds=ids((assignments||[]).map(a=>a.supplier_id));const vehicleIds=ids((assignments||[]).map(a=>a.vehicle_id));const peopleIds=ids((assignments||[]).flatMap(a=>[a.guide_person_id,a.driver_person_id]));const productIds=ids((services||[]).map(s=>s.product_catalog_id));
    const [supplierRes,vehicleRes,peopleRes,productRes]=await Promise.all([supplierIds.length?admin.from('suppliers').select('*').in('id',supplierIds):Promise.resolve({data:[]}),vehicleIds.length?admin.from('vehicles').select('*').in('id',vehicleIds):Promise.resolve({data:[]}),peopleIds.length?admin.from('service_people').select('*').in('id',peopleIds):Promise.resolve({data:[]}),productIds.length?admin.from('product_catalog').select('id,name,code,product_slug,stops,category').in('id',productIds):Promise.resolve({data:[]})]);
    ...
    const workbook=new ExcelJS.Workbook();await workbook.xlsx.load(Buffer.from(await templateBlob.arrayBuffer()));const datos=workbook.getWorksheet('DATOS PAX');if(datos)fillDatosPax(datos,passengers||[],lead);
    ...
    const tourContexts=serviceContexts.filter(ctx=>!isTransfer(ctx.text));const d80Base=workbook.getWorksheet('D80');if(d80Base){if(!tourContexts.length)workbook.removeWorksheet(d80Base.id);else{const sheets=makeCopies(workbook,d80Base,tourContexts.length,'D80');sheets.forEach((sheet,index)=>{const ctx=tourContexts[index];if(ctx)fillD80(sheet,ctx.service,ctx.pax,ctx,lead);});}}
    for(const sheetName of ['LUNA','CHAXA','SOCAIRE','TALABRE','COYO','TRF']){
      const base=workbook.getWorksheet(sheetName);if(!base)continue;const matches=serviceContexts.filter(item=>matchSheet(sheetName,item.text));if(!matches.length){workbook.removeWorksheet(base.id);continue;}const sheets=makeCopies(workbook,base,matches.length,sheetName);sheets.forEach((sheet,index)=>{const ctx=matches[index];if(!ctx)return;if(sheetName==='LUNA')fillLuna(sheet,ctx.service,ctx.pax,ctx);if(sheetName==='CHAXA')fillChaxa(sheet,ctx.service,ctx.pax,ctx);if(sheetName==='SOCAIRE')fillSocaire(sheet,ctx.service,ctx.pax,ctx,lead);if(sheetName==='TALABRE')fillTalabre(sheet,ctx.service,ctx.pax,ctx,lead);if(sheetName==='COYO')fillCoyo(sheet,ctx.service,ctx.pax,ctx,lead);if(sheetName==='TRF')fillTransfer(sheet,ctx.service,ctx.pax,ctx,lead);});
    }
    fillRiskPackets(workbook,workbook.getWorksheet('HOJA DE RIESGO'),tourContexts,lead);for(const name of ['RESERVA','ITINERARIO','LISTA MAQUETA','TEXTOS','INCIDENTES.','VOUCHER']){const sheet=workbook.getWorksheet(name);if(sheet)workbook.removeWorksheet(sheet.id);}
    const output=Buffer.from(await workbook.xlsx.writeBuffer());const {data:versionRows}=await admin.from('reservation_document_versions').select('version').eq('lead_id',leadId).eq('document_type','operation_sheet').order('version',{ascending:false}).limit(1);const version=Number(versionRows?.[0]?.version||0)+1;const safeCode=String(lead.codigo||lead.id).replace(/[^A-Za-z0-9_-]/g,'_');const fileName=`${safeCode}_OPERACION_v${version}.xlsx`;const storagePath=`generated/${safeCode}/${fileName}`;const {error:uploadError}=await admin.storage.from(BUCKET).upload(storagePath,output,{contentType:MIME,upsert:false,cacheControl:'0'});if(uploadError)throw uploadError;await admin.from('reservation_document_versions').update({status:'Archivado'}).eq('lead_id',leadId).eq('document_type','operation_sheet').eq('status','Activo');const {error:docError}=await admin.from('reservation_document_versions').insert({lead_id:leadId,document_type:'operation_sheet',title:`Paquete operacional ${lead.codigo} v${version}`,url:`storage://${BUCKET}/${storagePath}`,storage_bucket:BUCKET,storage_path:storagePath,version,status:'Activo',notes:'Generado automáticamente desde Supabase usando la plantilla maestra de HOTEL EXPERIENCE.',created_by:user.id});if(docError)throw docError;res.setHeader('Content-Type',MIME);res.setHeader('Content-Disposition',`attachment; filename="${fileName}"`);res.setHeader('X-Document-Version',String(version));return res.status(200).send(output);
