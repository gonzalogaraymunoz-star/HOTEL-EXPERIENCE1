...
function detectResponseMode(message,leadId){
  ...
  if(/\b(anomalia|anomalía|anomalias|anomalías|contradiccion|contradicción|contradicciones|inconsistencia|inconsistencias|audita|auditar|auditoria|auditoría|control preventivo|errores de datos)\b/.test(text))return 'control';
...
function responseContract(mode){
  ...
  if(mode==='control')return common+`\nESTRUCTURA DE CONTROL PREVENTIVO:\n## Riesgos críticos\nSolo contradicciones o conflictos con capacidad real de afectar una salida, cobro, proveedor o cierre.\n\n## Advertencias\nMáximo 5 inconsistencias que conviene corregir aunque no bloqueen hoy.\n\n## Señales de datos\nPosibles duplicados o datos dudosos deben llamarse “a revisar”, nunca errores confirmados.\n\n## Orden recomendado\nIndica qué corregir primero y por qué.\n\nREGLA: el motor de control determinístico es la fuente para saber qué anomalías fueron detectadas. No inventes anomalías adicionales como hechos. Puedes inferir impacto, pero identifícalo como inferencia.`;
...
function buildBusinessContext({mode,profile,products,tvPricing,lead,services,recentLeads,tasks,operational,control}){
  const privileged=profile.role==='admin'||profile.role==='manager';
  ...
  if(mode==='control')return {control_preventivo:control,lead_en_contexto:lead?compactLead(lead):null};
...
export default async function handler(req,res){
  ...
  try{
    ...
    const control=mode==='control'?await runControlChecks(admin,profile,user):null;
    ...
    const businessContext=buildBusinessContext({mode,profile,products:products||[],tvPricing,lead,services,recentLeads,tasks,operational,control});
    ...
    const temperature=mode==='writing'?0.35:0.15;const maxTokens=['executive','operational','control'].includes(mode)?1600:1200;
