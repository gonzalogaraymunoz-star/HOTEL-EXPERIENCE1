...
export default function ReservationOperations({lead,services,userRole,onChanged}:{lead:Lead;services:LeadService[];userRole:string;onChanged:()=>void}){
  ...
  return <div className="reservation-ops">
    ...
    <section className="ops-block">
      <div className="ops-head"><div><span className="eyebrow">OPERACIÓN POR TOUR</span><h3>Proveedor, prestadores, patente y costo</h3></div></div>
      <div className="assignment-stack">
        {services.map(service=>{
          ...
          return <article className="assignment-card" key={service.id}>
            ...
            <div className="assignment-grid">
              <label><span>Agencia / proveedor responsable</span><select disabled={!canEdit} value={a.supplier_id||''} onChange={async e=>{await updateServiceAssignment(service.id,{supplier_id:e.target.value||null});await load();onChanged()}}><option value="">Sin asignar</option>{data.suppliers.map((s:Supplier)=><option key={s.id} value={s.id}>{s.name} · {s.supplier_type}</option>)}</select></label>
              <label><span>Vehículo / patente</span><select disabled={!canEdit} value={a.vehicle_id||''} onChange={async e=>{const v=data.vehicles.find((x:Vehicle)=>x.id===e.target.value);await updateServiceAssignment(service.id,{vehicle_id:e.target.value||null,driver_person_id:a.driver_person_id||v?.driver_person_id||null});await load();onChanged()}}><option value="">Sin asignar</option>{vehicles.map((v:Vehicle)=><option key={v.id} value={v.id}>{v.plate} · {v.label} · {v.capacity||'?'} pax</option>)}</select></label>
              <label><span>Guía</span><select disabled={!canEdit} value={a.guide_person_id||''} onChange={async e=>{const person=people.find(p=>p.id===e.target.value);await updateServiceAssignment(service.id,{guide_person_id:e.target.value||null,guide_name:person?.full_name||null});await load();onChanged()}}><option value="">Sin asignar</option>{people.filter(p=>p.person_type==='Guía'||p.person_type==='Guía de montaña').map(p=>{const org=data.suppliers.find((s:Supplier)=>s.id===p.supplier_id);return <option key={p.id} value={p.id}>{p.full_name} · {p.person_type}{org?` · ${org.name}`:''}</option>})}</select></label>
              <label><span>Conductor</span><select disabled={!canEdit} value={a.driver_person_id||''} onChange={async e=>{const person=people.find(p=>p.id===e.target.value);await updateServiceAssignment(service.id,{driver_person_id:e.target.value||null,driver_name:person?.full_name||null});await load();onChanged()}}><option value="">Sin asignar</option>{people.filter(p=>p.person_type==='Conductor').map(p=>{const org=data.suppliers.find((s:Supplier)=>s.id===p.supplier_id);return <option key={p.id} value={p.id}>{p.full_name}{org?` · ${org.name}`:''}</option>})}</select></label>
              <label><span>Pickup</span><input disabled={!canEdit} type="time" value={a.pickup_time?.slice(0,5)||''} onChange={async e=>{await updateServiceAssignment(service.id,{pickup_time:e.target.value||null});await load();onChanged()}}/></label>
              <label><span>Punto de encuentro</span><input disabled={!canEdit} defaultValue={a.meeting_point||''} onBlur={async e=>{await updateServiceAssignment(service.id,{meeting_point:e.target.value||null});await load();onChanged()}} placeholder="Hotel / recepción / dirección"/></label>
              <label><span>Cocina / alimentación</span><select disabled={!canEdit} value={a.cook_person_id||''} onChange={async e=>{await updateServiceAssignment(service.id,{cook_person_id:e.target.value||null});await load();onChanged()}}><option value="">No aplica / sin asignar</option>{people.filter(p=>p.person_type==='Cocinero/a').map(p=><option key={p.id} value={p.id}>{p.full_name}</option>)}</select></label>
              <label><span>Coordinación terreno</span><select disabled={!canEdit} value={a.coordinator_person_id||''} onChange={async e=>{await updateServiceAssignment(service.id,{coordinator_person_id:e.target.value||null});await load();onChanged()}}><option value="">Sin asignar</option>{people.filter(p=>p.person_type==='Coordinador/a').map(p=><option key={p.id} value={p.id}>{p.full_name}</option>)}</select></label>
              <label><span>Costo proveedor</span><input disabled={!canEdit} inputMode="numeric" defaultValue={a.supplier_cost||0} onBlur={async e=>{await updateServiceAssignment(service.id,{supplier_cost:Number(e.target.value.replace(/\./g,''))||0});await load();onChanged()}}/></label>
              <label><span>Pago proveedor</span><select disabled={!canEdit} value={a.supplier_payment_status||'Pendiente'} onChange={async e=>{await updateServiceAssignment(service.id,{supplier_payment_status:e.target.value,supplier_payment_date:e.target.value==='Pagado'?new Date().toISOString():null});await load();onChanged()}}><option>Pendiente</option><option>Programado</option><option>Pagado</option><option>Disputado</option></select></label>
            </div>

            <div className="operation-summary-line">
              {supplier?<><b>{supplier.name}</b>{supplier.phone&&<a href={`tel:${supplier.phone}`}><Phone size={13}/>{supplier.phone}</a>}</>:<span>Proveedor pendiente</span>}
